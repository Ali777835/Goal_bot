...
bot.action(/^cancel_bot_(\d+\.\d+)$/, (ctx) => {
    const gameId = parseFloat(ctx.match[1]);
    const userId = ctx.from.id;
    const game = games[gameId];

    if (!game) return ctx.reply('❌ این بازی وجود ندارد.');
    if (game.creator !== 'bot') return ctx.reply('❌ فقط سازنده بازی می‌تواند آن را لغو کند.');

    getUser(userId, (user) => {
        updateUser(userId, { diamonds: user.diamonds + game.amount }, () => {
            delete games[gameId];
            ctx.reply('❌ بازی با ربات لغو شد. الماس شما برگردانده شد.');
        });
    });
});
